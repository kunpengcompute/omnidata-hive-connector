...
case class NdpPushDown(sparkSession: SparkSession)
  ...
  private val udfWhiteList = sparkUdfWhiteList
  ...
  private val saslEnable = NdpConf.getNdpZookeeperSaslEnable(sparkSession)
  private val jaasConf = NdpConf.getNdpZookeeperJaas(sparkSession)
  private val krb5Conf = NdpConf.getNdpZookeeperKrb5(sparkSession)
  ...
  def shouldPushDown(): Boolean = {
    ...
    fpuHosts = pushDownManagerClass.getZookeeperData(timeOut, parentPath, zkAddress, saslEnable, jaasConf, krb5Conf)
    fpuHosts.nonEmpty
  }
  ...
  def shouldPushDown(relation: HadoopFsRelation, output: Seq[Attribute]): Boolean = {
    ...
    var selectRatio = outputColumnTypeSize * 1.0 / tableColumnTpyeSize
    if (selectRatio < 0.0001) {
      selectRatio = 0.0001
    }
    ...
    isSupportFormat && relation.sizeInBytes > tableSizeThreshold.toLong &&
      selectRatio * relation.sizeInBytes < maxNdpTableSize
  }
      ...
      case f @ FilterExec(condition, s: NdpScanWrapper) if shouldPushDown(f, s.scan) =>
        ...
        val bb = s.logicalLink.get.children.map{ x =>
          if (x.isInstanceOf[Filter]) {
            val cc = FilterEstimation(x.asInstanceOf[Filter])
              .calculateFilterSelectivity(x.asInstanceOf[Filter].condition)
            selectivity2 = cc
          }
        }
...
case class NdpScanWrapper(scan: NdpSupport,
...
object NdpConf {
  ...
  val NDP_ZOOKEEPER_SASL_ENABLE = "spark.sql.ndp.zookeeper.sasl.client"
  val NDP_ZOOKEEPER_JAAS = "spark.sql.ndp.zookeeper.jaas.conf"
  val NDP_ZOOKEEPER_KRB5 = "spark.sql.ndp.zookeeper.krb5.conf"

  def getNdpZookeeperSaslEnable(sparkSession: SparkSession): Boolean = {
    toBoolean(NDP_ZOOKEEPER_SASL_ENABLE,
      sparkSession.conf.getOption(NDP_ZOOKEEPER_SASL_ENABLE).getOrElse("false"), sparkSession)
  }

  def getNdpZookeeperJaas(sparkSession: SparkSession): String = {
    sparkSession.conf.getOption(NDP_ZOOKEEPER_JAAS).getOrElse("/opt/boostkit/jaas.conf")
  }

  def getNdpZookeeperKrb5(sparkSession: SparkSession): String = {
    sparkSession.conf.getOption(NDP_ZOOKEEPER_KRB5).getOrElse("/opt/boostkit/krb5.conf")
  }
