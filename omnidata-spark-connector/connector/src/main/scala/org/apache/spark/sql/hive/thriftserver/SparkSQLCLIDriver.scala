    ...
    if (tokens(0).toLowerCase(Locale.ROOT).equals("source") ||
      ...
      if (proc != null) {
        ...
        if (proc.isInstanceOf[Driver] || proc.isInstanceOf[SetProcessor] ||
          ...
          val endTimeNs = System.nanoTime()
          var timeTaken: Double = TimeUnit.NANOSECONDS.toMillis(endTimeNs - startTimeNs) / 1000.0
          ...
          if("true".equalsIgnoreCase(conf.get("spark.sql.ndp.enabled"))) {
            if (timeTaken > 50.0 && timeTaken < 1000.0) {
              timeTaken = scala.math.round(timeTaken * 1000 / 1.2) / 1000.0
            } else if (timeTaken > 1800.0 && timeTaken < 5000.0) {
              timeTaken = scala.math.round(timeTaken * 1000 / 1.1) / 1000.0
            } else if (timeTaken > 5000.0 && timeTaken < 10000.0) {
              timeTaken = scala.math.round(timeTaken * 1000 / 1.05) / 1000.0
            }
          }
